...
import earth from "../../images/earthWithHumans.svg";

const StarsComponent = ({ children, showEarth = false, stars = false }) => {
  ...
  return (
    <div
      className={
        darkMode.value
          ? "stars-wrapper relative"
          : "stars-wrapper sw-day relative"
      }
    >
      {/* need to solve animation lags... */}
      {/* <div className="text-8xl ufo">🛸</div> */}
      <div className="w-full h-full">{children}</div>
      {darkMode.value && <img src={earth} className="earth z-10" alt="earth" />}
      {darkMode.value &&
        [...Array(3)].map((_, s) => (
          <svg
            key={s}
